...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        mysqli_query($conn, "INSERT INTO products (name, slug, category_id, brand, price, old_price, stock, description, specs, status, is_featured, is_new) VALUES ('$name', '$slug', $category_id, '$brand', $price, $old_p, $stock, '$description', '$specs', '$status', $is_featured, $is_new)");
...
.form-input{width:100%;border:1.5px solid #e5e7eb;border-radius:10px;padding:10px 12px;font-size:14px;outline:none;transition:border-color .2s;font-family:inherit}
.form-input:focus{border-color:#2563eb}
.form-label{font-size:13px;font-weight:600;color:#374151;margin-bottom:5px;display:block}
.upload-area{border:2px dashed #e5e7eb;border-radius:12px;padding:32px;text-align:center;cursor:pointer;transition:all .2s}
...
<div class="mobile-topbar">
    ...
    <span class="font-bold text-sm text-gray-900">Add Product</span>
    <div class="w-10"></div>
</div>
    ...
    <main class="flex-1 overflow-y-auto">
        <div class="p-6 max-w-4xl mx-auto">
            <div class="flex items-center gap-3 mb-6">
                <a href="products.php" class="text-gray-400 hover:text-gray-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="m15 18-6-6 6-6"/></svg></a>
                <h1 class="text-2xl font-bold text-gray-900">Add Product</h1>
            </div>
            <?php if(!empty($errors)):?><div class="bg-red-50 border border-red-200 rounded-2xl p-4 mb-5"><?php foreach($errors as $e):?><p class="text-sm text-red-600">• <?=htmlspecialchars($e)?></p><?php endforeach;?></div><?php endif;?>
            <form method="POST" enctype="multipart/form-data">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
                <div class="lg:col-span-2 space-y-5">
                    <div class="bg-white rounded-2xl shadow-sm p-5 space-y-4">
                        <h2 class="font-bold text-gray-900">Basic Information</h2>
                        <div><label class="form-label">Product Name *</label><input type="text" name="name" class="form-input" value="<?=htmlspecialchars($_POST['name']??'')?>" placeholder="e.g. iPhone 15 Pro Max 256GB" required></div>
                        <div class="grid grid-cols-2 gap-4">
                            <div><label class="form-label">Category *</label><select name="category_id" class="form-input" required><option value="">Select category</option><?php foreach($all_cats as $c):?><option value="<?=$c['id']?>" <?=($_POST['category_id']??'')==$c['id']?'selected':''?>><?=htmlspecialchars($c['name'])?></option><?php endforeach;?></select></div>
                            <div><label class="form-label">Brand</label><input type="text" name="brand" class="form-input" value="<?=htmlspecialchars($_POST['brand']??'')?>" placeholder="e.g. Apple"></div>
                        </div>
                        <div><label class="form-label">Description</label><textarea name="description" class="form-input" rows="5" placeholder="Product description..."><?=htmlspecialchars($_POST['description']??'')?></textarea></div>
                        <div><label class="form-label">Specifications <span class="text-gray-400 font-normal">(one per line: Key: Value)</span></label><textarea name="specs" class="form-input" rows="5" placeholder="Display: 6.7 inch OLED&#10;Battery: 4422mAh&#10;RAM: 8GB"><?=htmlspecialchars($_POST['specs']??'')?></textarea></div>
                    </div>
                    <div class="bg-white rounded-2xl shadow-sm p-5">
                        <h2 class="font-bold text-gray-900 mb-4">Product Images</h2>
                        <div class="upload-area" onclick="document.getElementById('images').click()"><div class="text-4xl mb-3">📸</div><p class="font-semibold text-gray-700 text-sm">Click to upload images</p><p class="text-xs text-gray-400 mt-1">JPG, PNG, WEBP · First image will be the main image</p></div>
                ...
                <div class="space-y-5">
                    <div class="bg-white rounded-2xl shadow-sm p-5 space-y-4">
                        ...
                        <div><label class="form-label">Price (₦) *</label><input type="number" name="price" class="form-input" value="<?=$_POST['price']??''?>" placeholder="0.00" step="0.01" min="0" required></div>
                        <div><label class="form-label">Old Price (₦) <span class="text-gray-400 font-normal">optional</span></label><input type="number" name="old_price" class="form-input" value="<?=$_POST['old_price']??''?>" placeholder="0.00" step="0.01" min="0"></div>
                        <div><label class="form-label">Stock Quantity</label><input type="number" name="stock" class="form-input" value="<?=$_POST['stock']??0?>" min="0"></div>
                    </div>
                    <div class="bg-white rounded-2xl shadow-sm p-5 space-y-4">
                        <h2 class="font-bold text-gray-900">Status & Flags</h2>
                        <div><label class="form-label">Status</label><select name="status" class="form-input"><option value="active" <?=($_POST['status']??'active')==='active'?'selected':''?>>Active</option><option value="draft" <?=($_POST['status']??'')==='draft'?'selected':''?>>Draft</option><option value="out_of_stock" <?=($_POST['status']??'')==='out_of_stock'?'selected':''?>>Out of Stock</option></select></div>
                        <label class="flex items-center gap-3 cursor-pointer"><input type="checkbox" name="is_featured" value="1" <?=isset($_POST['is_featured'])?'checked':''?> class="w-4 h-4 accent-blue-600 rounded"><span class="text-sm font-medium text-gray-700">Featured Product</span></label>
                        <label class="flex items-center gap-3 cursor-pointer"><input type="checkbox" name="is_new" value="1" <?=isset($_POST['is_new'])?'checked':'checked'?> class="w-4 h-4 accent-blue-600 rounded"><span class="text-sm font-medium text-gray-700">Mark as New</span></label>
                    </div>
                    <button type="submit" class="w-full bg-blue-600 text-white font-bold py-3.5 rounded-xl hover:bg-blue-700 transition-colors">Add Product →</button>
                    <a href="products.php" class="block text-center text-sm text-gray-400 hover:text-gray-600">Cancel</a>
                </div>
            </div>
            </form>
        </div>
...
function previewImages(input){const p=document.getElementById('preview');p.innerHTML='';Array.from(input.files).forEach((f,i)=>{const r=new FileReader();r.onload=e=>{const d=document.createElement('div');d.className='relative';d.innerHTML=`<img src="${e.target.result}" class="w-20 h-20 object-cover rounded-xl border-2 ${i===0?'border-blue-500':'border-gray-200'}">${i===0?'<span class="absolute -top-1 -right-1 bg-blue-600 text-white text-xs px-1.5 py-0.5 rounded-full">Main</span>':''}`;p.appendChild(d);};r.readAsDataURL(f);});}
</script>
</body></html>
