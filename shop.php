...
.filter-chip {
    ...
    background: white; color: #1d1d1f; white-space: nowrap;
    text-decoration: none;
}
.filter-chip:hover, .filter-chip.active { background: #0071e3; color: white; border-color: #0071e3; }
.sidebar-section { border-bottom: 1px solid #f0f0f0; padding-bottom: 20px; margin-bottom: 20px; }
.section-eyebrow { font-size: 12px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: #0071e3; }
.search-bar {
...
<nav class="navbar-blur fixed top-0 left-0 right-0 z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14">
        ...
        <a href="index.php" class="flex items-center gap-2 flex-shrink-0">
            <img src="assets/images/logo.png" alt="Frank Gadgets" style="height:40px;width:auto;max-width:40px;">
            <span style="font-weight:700;font-size:15px;color:#1d1d1f;white-space:nowrap;">Frank Gadgets</span>
        </a>
    ...
    <div id="mobile-menu" class="hidden md:hidden bg-white border-t border-gray-100 px-4 py-4">
        <div class="flex flex-col gap-1">
            ...
            <a href="shop.php?category=<?php echo $c['slug']; ?>"
               class="text-sm text-fg-gray py-2.5 px-3 rounded-xl border-b border-gray-50 <?php echo $c['slug']===$cat_slug?'text-fg-blue font-semibold':'' ?>">
                <?php echo htmlspecialchars($c['name']); ?>
            </a>
            <?php endforeach; ?>
            <a href="shop.php" class="text-sm text-fg-blue font-medium py-2.5 px-3 mt-1">View All Products →</a>
            <div class="pt-2">
...
<div class="pt-14 bg-fg-light">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        <p class="section-eyebrow mb-2"><?php echo $search ? 'Search Results' : ($cat_slug ? 'Browse' : 'Store'); ?></p>
...
<div class="bg-white border-b border-gray-100 sticky top-14 z-40">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-2 overflow-x-auto no-scrollbar">
        <a href="shop.php<?php echo $search?'?q='.urlencode($search):''; ?>" class="filter-chip <?php echo !$cat_slug?'active':''; ?>">All</a>
        <?php foreach($all_cats as $c): ?>
    ...
    <div class="flex gap-8">
        ...
        <aside class="hidden lg:block w-56 flex-shrink-0">
            <div class="sidebar-section">
                ...
                ?>
                <a href="shop.php?<?php echo http_build_query($params); ?>"
                   class="flex items-center gap-2 py-2 text-sm <?php echo $sort===$val?'text-fg-blue font-semibold':'text-fg-gray hover:text-fg-dark'; ?> transition-colors">
                    ...
                    <?php else: ?><span class="w-3.5 h-3.5 inline-block"></span><?php endif; ?>
                    <?php echo $label; ?>
                </a>
                <?php endforeach; ?>
            </div>
                ...
                <form method="GET">
                    ...
                    <div class="flex gap-2 mb-3">
                        <div class="flex-1">
                            ...
                            <input type="number" name="min" value="<?php echo $min_p?:''; ?>" placeholder="0"
                        ...
                        <div class="flex-1">
                            ...
                            <input type="number" name="max" value="<?php echo $max_p?:''; ?>" placeholder="Any"
                                class="w-full border border-gray-200 rounded-lg px-2 py-1.5 text-sm outline-none focus:border-fg-blue">
                        </div>
                    </div>
                    <button type="submit" class="w-full bg-fg-blue text-white py-2 rounded-xl text-sm font-medium hover:bg-fg-blue-dark transition-colors">Apply</button>
                    <?php if($min_p || $max_p): ?>
                    <a href="shop.php?<?php echo http_build_query(array_diff_key($_GET,['min'=>'','max'=>'','page'=>''])); ?>"
                       class="block text-center text-xs text-fg-gray hover:text-fg-blue mt-2">Clear filter</a>
                    <?php endif; ?>
                </form>
            </div>
            ...
            <div>
                ...
                <?php foreach($all_cats as $c): ?>
                <a href="shop.php?category=<?php echo $c['slug']; ?>"
                   class="block py-2 text-sm <?php echo $c['slug']===$cat_slug?'text-fg-blue font-semibold':'text-fg-gray hover:text-fg-dark'; ?> transition-colors">
                    <?php echo htmlspecialchars($c['name']); ?>
                </a>
                <?php endforeach; ?>
                <a href="shop.php" class="block py-2 text-sm <?php echo !$cat_slug?'text-fg-blue font-semibold':'text-fg-gray hover:text-fg-dark'; ?> transition-colors">All Products</a>
            </div>
        </aside>
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center justify-between mb-6 gap-4">
                <p class="text-sm text-fg-gray">
                    Showing <span class="font-semibold text-fg-dark"><?php echo min($offset+1,$total_products); ?>–<?php echo min($offset+$per_page,$total_products); ?></span>
                    of <span class="font-semibold text-fg-dark"><?php echo $total_products; ?></span>
                </p>
                <div class="flex items-center gap-3">
                    <button onclick="document.getElementById('filter-drawer').classList.toggle('-translate-x-full'); document.getElementById('drawer-overlay').classList.toggle('hidden')"
                    ...
                    <select onchange="window.location=this.value"
                        ...
                        <?php foreach($sorts as $val => $label):
                            $params = array_merge($_GET, ['sort'=>$val,'page'=>1]);
                        ?>
                        <option value="shop.php?<?php echo http_build_query($params); ?>" <?php echo $sort===$val?'selected':''; ?>>
                            <?php echo $label; ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="text-center py-24">
                ...
                <a href="shop.php" class="bg-fg-blue text-white px-8 py-3 rounded-full font-medium text-sm hover:bg-fg-blue-dark transition-colors">Browse All Products</a>
            </div>
            <?php else: ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                <?php while($p = mysqli_fetch_assoc($products_res)):
                    ...
                    $discount = ($p['old_price'] && $p['old_price'] > $p['price'])
                        ? round((($p['old_price'] - $p['price']) / $p['old_price']) * 100) : 0;
                ?>
                   ...
                   class="product-card bg-white border border-gray-100 rounded-3xl overflow-hidden block relative group shadow-sm">
                    <div class="relative bg-fg-light overflow-hidden" style="height:210px">
                        ...
                        <div class="absolute top-3 left-3 flex flex-col gap-1">
                            <?php if($p['is_new']): ?><span class="bg-fg-blue text-white text-xs font-semibold px-2 py-0.5 rounded-full">New</span><?php endif; ?>
                            <?php if($p['is_featured']): ?><span class="bg-amber-400 text-white text-xs font-semibold px-2 py-0.5 rounded-full">Featured</span><?php endif; ?>
                            <?php if($discount > 0): ?><span class="bg-red-500 text-white text-xs font-semibold px-2 py-0.5 rounded-full">-<?php echo $discount; ?>%</span><?php endif; ?>
                        </div>
                    ...
                    <div class="p-4">
                        ...
                        <h3 class="font-semibold text-sm leading-tight mb-3 line-clamp-2 text-fg-dark"><?php echo htmlspecialchars($p['name']); ?></h3>
            ...
            <div class="flex items-center justify-center gap-2 mt-12">
                ...
                <a href="shop.php?<?php echo http_build_query(array_merge($_GET,['page'=>$page-1])); ?>"
                   class="w-10 h-10 flex items-center justify-center border border-gray-200 rounded-xl hover:border-fg-blue hover:text-fg-blue transition-colors text-sm">‹</a>
                <?php endif; ?>
                <?php for($i=max(1,$page-2); $i<=min($total_pages,$page+2); $i++): ?>
                <a href="shop.php?<?php echo http_build_query(array_merge($_GET,['page'=>$i])); ?>"
                   class="w-10 h-10 flex items-center justify-center rounded-xl text-sm font-medium transition-colors <?php echo $i===$page?'bg-fg-blue text-white':'border border-gray-200 hover:border-fg-blue hover:text-fg-blue'; ?>">
                    <?php echo $i; ?>
                </a>
                <?php endfor; ?>
...
<div id="filter-drawer" class="fixed inset-y-0 left-0 w-72 bg-white z-[60] shadow-2xl -translate-x-full transform overflow-y-auto p-6 lg:hidden">
    <div class="flex items-center justify-between mb-6">
        ...
        <button onclick="document.getElementById('filter-drawer').classList.add('-translate-x-full'); document.getElementById('drawer-overlay').classList.add('hidden')"
            class="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 text-lg">×</button>
    </div>
    <div class="sidebar-section">
        ...
        ?>
        <a href="shop.php?<?php echo http_build_query($params); ?>"
           class="block py-2 text-sm <?php echo $sort===$val?'text-fg-blue font-semibold':'text-fg-gray'; ?>">
            <?php echo $label; ?>
        </a>
        <?php endforeach; ?>
    </div>
...
<div id="drawer-overlay" class="fixed inset-0 bg-black/40 z-50 hidden lg:hidden"
    onclick="document.getElementById('filter-drawer').classList.add('-translate-x-full'); this.classList.add('hidden')"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-10 mb-12">
            ...
            <div class="col-span-2 md:col-span-1">
                <div class="flex items-center gap-2 mb-4">
                    <img src="assets/images/logo.png" alt="Frank Gadgets" style="height:40px;width:auto;max-width:40px;">
                    <span style="font-weight:700;font-size:15px;color:white;white-space:nowrap;">Frank Gadgets</span>
                </div>
                <p class="text-gray-400 text-sm leading-relaxed">Nigeria's most trusted premium gadget store. Quality tech, delivered to you.</p>
            </div>
            ...
            <div>
                ...
                <div class="flex flex-col gap-2">
                    <?php foreach(array_slice($all_cats,0,5) as $c): ?>
                    <a href="shop.php?category=<?php echo $c['slug']; ?>" class="text-gray-400 text-sm hover:text-white transition-colors">
                        <?php echo htmlspecialchars($c['name']); ?>
                    </a>
                    <?php endforeach; ?>
                </div>
            ...
            <div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <a href="#" class="text-gray-400 text-sm hover:text-white">Warranty</a>
        ...
        <div class="border-t border-gray-800 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="#" class="text-gray-600 text-xs hover:text-gray-400">Privacy</a>
                <a href="#" class="text-gray-600 text-xs hover:text-gray-400">Terms</a>
            </div>
        </div>
    </div>
</footer>
